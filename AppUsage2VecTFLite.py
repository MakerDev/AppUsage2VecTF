...
class AppUsage2VecTFLite(keras.models.Model):
    def __init__(self, n_users, n_apps, dim, seq_length, n_layers, alpha, k):
        ...
        self.user_dnn = [tf.keras.layers.Dense(dim, input_shape=(dim,)) for _ in range(n_layers)]
        self.app_dnn = [tf.keras.layers.Dense(dim, input_shape=(dim,)) for _ in range(n_layers)]
    ...
    def call(self, users, time_vecs, app_seqs, time_seqs, targets, mode='test'):
        ...
        """
        scores = self.predict_batch(app_seqs, time_seqs, users, time_vecs)
        ...
        loss = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=scores, labels=tf.reshape(targets, [-1])))
    ...
    def predict_batch(self, app_seqs, time_seqs, users, time_vecs):
        ...
        seq_vector = tf.squeeze(tf.matmul(tf.transpose(app_seqs_emb, [0, 2, 1]), tf.expand_dims(weights, axis=2)),
                                axis=2)
